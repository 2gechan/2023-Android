...
/// provider를 활용한 MVVM 패턴에서는 state 값을 변화시키는 코드를
...
class TimerViewModel extends ChangeNotifier {
  ...
  /// 클래스로 객체(인스턴스)를 생성하면 각각 생성되는 인스턴스는 독립적이다
  /// 객체 지향에서 한개의 객체(인스턴스)를 만들고, 프로젝트 전체에서
  /// 공통으로 공유를 해야 하는 경우가 있다.
  /// 그럴때 static을 사용하기도 하지만, 싱글톤이라는 패턴(기법)을 사용하여
  /// 이미 생성된 객체를 재활용(다시 이용)하는 방법이 있다.
  /// static(프로젝트 start 되면 자동으로 객체 생성)에 비해서
  /// 필요한 경우만 객체를 한번 생성하여 재활용 한다는 의미에서
  /// statc에 비하여 메모리 효율성을 기대할 수 있다.
...
// 변수 이름(식별자)에 _ 가 부착되면 이 변수는 private 성질을 띈다
// async 패키지에 포함된 객체
  late Timer _timer;
...
// 이 클래스에서 만들어진 _prefs 객체를 private으로 사용하기 위하여 요구하기
    ...
    // localStorage(SP)에 TIMER라는 이름으로 변수가 저장되어 있으면
    ...
    if (timerDto.timerRun) {
      ...
      _timer = Timer.periodic(
        ...
        (timer) {
          ...
          // viewModel 에서 state 변수 값을 변화 시켰으니
          // 화면에 그려라
